...
public class StarbasePhaserSection2 : MonoBehaviour {
	...
	//variable bools to track whether phasers have been used this turn
	...
	public static PhaserSectionDestroyedEvent OnPhaserSection2Destroyed = new PhaserSectionDestroyedEvent();

	//event to announce the section was repaired
	public static PhaserSectionDestroyedEvent OnPhaserSection2Repaired = new PhaserSectionDestroyedEvent();
	...
	public void Init () {
		...
		this.shieldsMax = Starbase.phaserSection2ShieldsMax;
		...
		//CombatManager.OnPhaserAttackHitBasePhaserSection2.AddListener(attackHitTakeDamageAction);
		CutsceneManager.OnPhaserHitBasePhaserSection2.AddListener(attackHitTakeDamageAction);

		//add listener for getting hit by torpedo attack
		//CombatManager.OnLightTorpedoAttackHitBasePhaserSection2.AddListener(attackHitTakeDamageAction);
		//CombatManager.OnHeavyTorpedoAttackHitBasePhaserSection2.AddListener(attackHitTakeDamageAction);
		CutsceneManager.OnLightTorpedoHitBasePhaserSection2.AddListener(attackHitTakeDamageAction);
		CutsceneManager.OnHeavyTorpedoHitBasePhaserSection2.AddListener(attackHitTakeDamageAction);

		//add listener for getting healed by a crystal
		CombatManager.OnCrystalUsedOnBasePhaserSection2.AddListener(crystalUsedHealDamageAction);

		//add listener for getting repaired by a repair crew
		CombatManager.OnRepairCrewUsedOnBasePhaserSection2.AddListener(repairCrewUsedOnThisRepairSectionAction);
	...
	private void FirePhasers(CombatUnit firingUnit, CombatUnit targetedUnit, string sectionTargeted){
		...
		if (this.GetComponent<CombatUnit> () == firingUnit) {
			...
			//check if the PhaserSection1 is still alive.  If it is, we will have that section invoke the event - we don't want to pass two OnFire events
			if (gameObject.GetComponent<StarbasePhaserSection1> ().isDestroyed == true) {
	...
	private void RepairSection(CombatUnit targetedUnit){
		...
		if (this.GetComponent<CombatUnit> () == targetedUnit) {
			...
			OnPhaserSection2Repaired.Invoke(this.GetComponent<CombatUnit>());
	...
	private void DestroySection (){
		...
		OnPhaserSection2Destroyed.Invoke(this.GetComponent<CombatUnit>());
		...
		if (this.GetComponent<CombatUnit> () == combatUnit) {
			...
			for (int i = 0; i < GameManager.numberPlayers; i++) {
				...
				if (saveGameData.playerColor [i] == combatUnit.owner.color) {
					...
					this.isDestroyed = saveGameData.starbasePhaserSection2IsDestroyed [i];
					this.shieldsCurrent = saveGameData.starbasePhaserSection2ShieldsCurrent [i];
					this.usedPhasersThisTurn = saveGameData.starbasePhaserSection2UsedPhasersThisTurn [i];
	...
	private void RemoveAllListeners(){
		...
		//CombatManager.OnPhaserAttackHitBasePhaserSection2.RemoveListener(attackHitTakeDamageAction);
		CutsceneManager.OnPhaserHitBasePhaserSection2.RemoveListener(attackHitTakeDamageAction);

		//remove listener for getting hit by torpedo attack
		//CombatManager.OnLightTorpedoAttackHitBasePhaserSection2.RemoveListener(attackHitTakeDamageAction);
		//CombatManager.OnHeavyTorpedoAttackHitBasePhaserSection2.RemoveListener(attackHitTakeDamageAction);
		CutsceneManager.OnLightTorpedoHitBasePhaserSection2.RemoveListener(attackHitTakeDamageAction);
		CutsceneManager.OnHeavyTorpedoHitBasePhaserSection2.RemoveListener(attackHitTakeDamageAction);

		//remove listener for getting healed by a crystal
		CombatManager.OnCrystalUsedOnBasePhaserSection2.RemoveListener(crystalUsedHealDamageAction);

		//remove listener for getting repaired by a repair crew
		CombatManager.OnRepairCrewUsedOnBasePhaserSection2.RemoveListener(repairCrewUsedOnThisRepairSectionAction);
