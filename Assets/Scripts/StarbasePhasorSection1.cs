...
public class StarbasePhasorSection1 : MonoBehaviour {
	...
	public static PhasorSectionDestroyedEvent OnPhasorSection1Destroyed = new PhasorSectionDestroyedEvent();

	//event to announce the section was repaired
	public static PhasorSectionDestroyedEvent OnPhasorSection1Repaired = new PhasorSectionDestroyedEvent();
	...
	public void Init () {
		...
		this.phasorRadarShot = Starbase.startingPhasorRadarShot;
		this.phasorRadarArray = Starbase.startingPhasorRadarArray;
		this.shieldsMax = Starbase.phasorSection1ShieldsMax;
		...
		//CombatManager.OnPhasorAttackHitBasePhasorSection1.AddListener(attackHitTakeDamageAction);
		CutsceneManager.OnPhasorHitBasePhasorSection1.AddListener(attackHitTakeDamageAction);

		//add listener for getting hit by torpedo attack
		//CombatManager.OnLightTorpedoAttackHitBasePhasorSection1.AddListener(attackHitTakeDamageAction);
		//CombatManager.OnHeavyTorpedoAttackHitBasePhasorSection1.AddListener(attackHitTakeDamageAction);
		CutsceneManager.OnLightTorpedoHitBasePhasorSection1.AddListener(attackHitTakeDamageAction);
		CutsceneManager.OnHeavyTorpedoHitBasePhasorSection1.AddListener(attackHitTakeDamageAction);

		//add listener for getting healed by a crystal
		CombatManager.OnCrystalUsedOnBasePhasorSection1.AddListener(crystalUsedHealDamageAction);

		//add listener for getting repaired by a repair crew
		CombatManager.OnRepairCrewUsedOnBasePhasorSection1.AddListener(repairCrewUsedOnThisRepairSectionAction);
	...
	private void RepairSection(CombatUnit targetedUnit){
		...
		if (this.GetComponent<CombatUnit> () == targetedUnit) {
			...
			OnPhasorSection1Repaired.Invoke(this.GetComponent<CombatUnit>());
	...
	private void DestroySection (){
		...
		OnPhasorSection1Destroyed.Invoke(this.GetComponent<CombatUnit>());
		...
		if (this.GetComponent<CombatUnit> () == combatUnit) {
			...
			for (int i = 0; i < GameManager.numberPlayers; i++) {
				...
				if (saveGameData.playerColor [i] == combatUnit.owner.color) {
					...
					this.isDestroyed = saveGameData.starbasePhasorSection1IsDestroyed [i];
					this.shieldsCurrent = saveGameData.starbasePhasorSection1ShieldsCurrent [i];
					this.usedPhasorsThisTurn = saveGameData.starbasePhasorSection1UsedPhasorsThisTurn [i];
					this.phasorRadarShot = saveGameData.starbasePhasorRadarShot [i];
					this.phasorRadarArray = saveGameData.starbasePhasorRadarArray [i];
	...
	private void RemoveAllListeners(){
		...
		//CombatManager.OnPhasorAttackHitBasePhasorSection1.RemoveListener (attackHitTakeDamageAction);
		CutsceneManager.OnPhasorHitBasePhasorSection1.RemoveListener(attackHitTakeDamageAction);

		//remove listener for getting hit by torpedo attack
		//CombatManager.OnLightTorpedoAttackHitBasePhasorSection1.RemoveListener (attackHitTakeDamageAction);
		//CombatManager.OnHeavyTorpedoAttackHitBasePhasorSection1.RemoveListener (attackHitTakeDamageAction);
		CutsceneManager.OnLightTorpedoHitBasePhasorSection1.RemoveListener(attackHitTakeDamageAction);
		CutsceneManager.OnHeavyTorpedoHitBasePhasorSection1.RemoveListener(attackHitTakeDamageAction);

		//remove listener for getting healed by a crystal
		CombatManager.OnCrystalUsedOnBasePhasorSection1.RemoveListener (crystalUsedHealDamageAction);

		//remove listener for getting repaired by a repair crew
		CombatManager.OnRepairCrewUsedOnBasePhasorSection1.RemoveListener (repairCrewUsedOnThisRepairSectionAction);
